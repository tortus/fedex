= FedEx
  ...
  NOTE: This version should support v8 of the FedEx API, although it has not been exhaustively tested (shipping yes, rate requests no).
  
  WARNING: Plugin isn't yet stable!! I'm in the process of supporting multiple packages and 
  international shipping, but until that's completed this branch is quite likely to be broken.

== Updates over other repos

  * Upgraded to v8, the latest version of the FedEx API as of May 2010
  * Extended to allow grouping multiple packages under a single master tracking number
  * WIP - extended to allow international shipping
  * A number of smaller changes, see git commits for details

== API Changes

This update is backwards-compatible with older versions; if you're only sending one item, you can still use the old-style inline API for package information:
...
Due to copyright reasons we cannot distribute the associated WSDL files; you will need to apply for a {developer account}[http://www.fedex.com/developer] with Fedex to begin working on your integration.  Once you've created your account, head to the "Get Started" section, where you can find documentation and the individual WSDLs for all of the available services.  For our purposes you need only two: Rate (<tt>RateService_v8.wsdl</tt> as of May 2010) and Ship (<tt>ShipService_v8.wsdl</tt> as of May 2010). Download these WSDLs and put them in the <tt>vendor/plugins/fedex/lib/wsdl/</tt> directory.
